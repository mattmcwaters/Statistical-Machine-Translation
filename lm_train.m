...
for iFile=1:length(DD)
  ...
  for l=1:length(lines)
    ...
    words = strsplit(' ', processedLine, 'omit');
    clearvars processedLine
    prevWord=SENTSTARTMARK;
 
    for iWord = 1:length(words)
        if length(words{iWord})<1
            disp('CAUGHTONE')
        else    
            %Uni
            if isfield(LM.uni, words{iWord})
                LM.uni.(words{iWord}) = LM.uni.(words{iWord}) +1;

            else
                LM.uni.(words{iWord}) = 1;
            end
            if iWord >1
                if isfield(LM.bi, prevWord)
                     if isfield(LM.bi.(prevWord), words{iWord})
                         LM.bi.(prevWord).(words{iWord}) = LM.bi.(prevWord).(words{iWord}) + 1;
                     else
                          LM.bi.(prevWord).(words{iWord})=1;
                     end 
                else
                    LM.bi.(prevWord).(words{iWord})=1;
                end  
                prevWord=words{iWord};
            end
        end
    end
